...
define('Mojo Helpers', [
  ...
  if (window.add_completion_callback) add_completion_callback(tearDown);
...
function loadMojoModules(name, modules = []) {
  return define('Mojo modules: ' + name, ['Mojo Helpers'].concat(modules), (
    mojo,
    ...rest
  ) => {
    mojo.modules = rest;
...
function mojo_test(func, name, properties) {
  promise_test(
    () =>
      loadMojoModules(name).then(mojo => {
