...
(function() {
  ...
  function assert_throws(code, func, description) {
    try {
      func.call(this);
      assert(false, 'assert_throws', description, '${func} did not throw', {
        func: func
      });
    } catch (e) {
      ...
      if (typeof code === 'object') {
        assert(
          typeof e == 'object' && 'name' in e && e.name == code.name,
          'assert_throws',
          description,
      ...
      for (var prop in required_props) {
        assert(
          typeof e == 'object' && prop in e && e[prop] == required_props[prop],
          'assert_throws',
          description,
  ...
  function RemoteWorker(worker) {
    ...
    message_port.onmessage = function(message) {
  ...
  Tests.prototype.all_done = function() {
    return (
      ...
      !this.pending_workers.some(function(w) {
        return w.running;
      })
    );
  };
  ...
  Output.prototype.show_results = function(tests, harness_status) {
    ...
    forEach(output_document.querySelectorAll('section#summary label'), function(
      element
    ) {
      on_event(element, 'click', function(e) {
        ...
        if (!style_element && !input_element.checked) {
          ...
          style_element.textContent =
            'table#results > tbody > tr.' + result_class + '{display:none}';
    ...
    var html =
      "<h2>Details</h2><table id='results' " +
    ...
    for (var i = 0; i < tests.length; i++) {
      html +=
        ...
        (tests[i].stack
          ? '<pre>' + escape_html(tests[i].stack) + '</pre>'
          : '') +
        '</td></tr>';
    }
    html += '</tbody></table>';
  ...
  function get_harness_url() {
    ...
    return script_url
      ? script_url.slice(0, script_url.lastIndexOf('/') + 1)
      : undefined;
  }
