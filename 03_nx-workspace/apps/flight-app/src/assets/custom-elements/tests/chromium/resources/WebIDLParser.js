(function() {
  ...
  var parse = function(tokens, opt) {
    ...
    var single_type = function() {
      ...
      type_suffix(ret);
      if (ret.nullable && !ret.array && ret.idlType === 'any')
    ...
    var argument_list = function(store) {
      ...
      while (true) {
        ...
        var nxt =
          argument(store ? ret : null) ||
          error('Trailing comma in arguments list');
        ret.push(nxt);
      }
    };
    ...
    var simple_extended_attr = function(store) {
      ...
      if (eq) {
        ...
        if ((rhs = consume(ID))) {
          ...
          consume(OTHER, ')') ||
            error(
              'Unexpected token in extended attribute argument list or type pair'
            );
          ret.rhs = {
    ...
    var default_ = function() {
      ...
      if (consume(OTHER, '=')) {
        ...
        if (def) {
          ...
          if (!consume(OTHER, ']'))
            error('Default sequence value must be empty');
    ...
    var iterable = function(store) {
      ...
      if (consume(OTHER, '<')) {
        ret.idlType =
          type() || error('Error parsing ' + ittype + ' declaration');
        all_ws();
        if (secondTypeAllowed) {
          ...
          if (type2) ret.idlType = [ret.idlType, type2];
          else if (secondTypeRequired)
            error('Missing second type argument in ' + ittype + ' declaration');
        }
        if (!consume(OTHER, '>'))
          error('Unterminated ' + ittype + ' declaration');
        all_ws();
        if (!consume(OTHER, ';'))
          error('Missing semicolon after ' + ittype + ' declaration');
    ...
    var interface_ = function(isPartial, store) {
      ...
      while (true) {
        ...
        var mem =
          (opt.allowNestedTypedefs && typedef(store ? mems : null)) ||
    ...
    var implements_ = function(store) {
      ...
      if (consume(ID, 'implements')) {
        ...
        all_ws();
        consume(OTHER, ';') ||
          error('No terminating ; for implements statement');
        return ret;
      } else {
        // rollback
        tokens.unshift(w);
