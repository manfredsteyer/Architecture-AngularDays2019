...
(function() {
  ...
  function getParentObject(ancestors) {
    ...
    ancestors.forEach(function(p) {
      ...
      if (parent[p] === undefined) {
        throw currentName +
          ' is undefined, cannot set prefix alias on child object';
      }
      parent = parent[p];
    });
    return parent;
      ...
      vendorPrefixes.forEach(function(prefix) {
        if (parent.prototype.hasOwnProperty(prependPrefix(prefix, obj.name))) {
          Object.defineProperty(parent.prototype, obj.name, {
            get: function() {
              return this[prependPrefix(prefix, obj.name)];
            },
            set: function(v) {
              this[prependPrefix(prefix, obj.name)] = v;
            }
          });
          aliases[obj.ancestors.join('.') + '.prototype.' + obj.name] =
            obj.ancestors.join('.') +
            '.prototype.' +
            prependPrefix(prefix, obj.name);
          return;
        }
      });
    }
    ...
    if (parent[obj.name] === undefined) {
      vendorPrefixes.forEach(function(prefix) {
        if (parent[prependPrefix(prefix, obj.name)] !== undefined) {
          ...
          aliases[obj.ancestors.join('.') + '.' + obj.name] =
            obj.ancestors.join('.') + '.' + prependPrefix(prefix, obj.name);
          return;
        }
      });
    }
  ...
  if (document.querySelector('script[data-prefixed-objects]')) {
    var prefixObjectsData = document.querySelector(
      'script[data-prefixed-objects]'
    ).dataset['prefixedObjects'];
    try {
  ...
  if (document.querySelector('script[data-prefixed-prototypes]')) {
    var prefixProtoData = document.querySelector(
      'script[data-prefixed-prototypes]'
    ).dataset['prefixedPrototypes'];
    try {
