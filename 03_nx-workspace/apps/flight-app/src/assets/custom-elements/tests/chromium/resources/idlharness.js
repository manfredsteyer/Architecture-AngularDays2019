...
(function() {
  ...
  function throwOrReject(a_test, operation, fn, obj, args, message, cb) {
    if (operation.idlType.generic !== 'Promise') {
      ...
      try {
        promise_rejects(a_test, new TypeError(), fn.apply(obj, args)).then(
          cb,
          cb
        );
      } catch (e) {
  ...
  function exposed_in(globals) {
    ...
    if (
      ...
      return (
        globals.indexOf('Worker') >= 0 || globals.indexOf('SharedWorker') >= 0
      );
    }
    if (
      ...
      return (
        globals.indexOf('Worker') >= 0 || globals.indexOf('ServiceWorker') >= 0
      );
    }
  ...
  IdlArray.prototype.test = function() //@{
  {
    ...
    this.partials.forEach(
      function(parsed_idl) {
        if (
          ...
          throw 'Partial interface ' +
            parsed_idl.name +
            ' with no original interface';
        }
    ...
    for (var lhs in this['implements']) {
      this.recursively_get_implements(lhs).forEach(
        function(rhs) {
    ...
    Object.getOwnPropertyNames(this.members).forEach(
      function(memberName) {
        ...
        var exposed = member.extAttrs.filter(function(a) {
          return a.name == 'Exposed';
        });
        if (exposed.length > 1) {
          throw 'Unexpected Exposed extended attributes on ' +
            memberName +
            ': ' +
            exposed;
        }

        var globals = exposed.length === 1 ? exposed[0].rhs.value : ['Window'];
  ...
  IdlArray.prototype.assert_type_is = function(value, type) //@{
  {
    ...
    switch (type) {
      ...
      case 'octet':
        ...
        assert_true(
          0 <= value && value <= 255,
          'octet ' + value + ' not in range [0, 255]'
        );
        return;
      ...
      case 'short':
        ...
        assert_true(
          ...
          'short ' + value + ' not in range [-32768, 32767]'
        );
        return;
      ...
      case 'unsigned short':
        ...
        assert_true(
          0 <= value && value <= 65535,
          'unsigned short ' + value + ' not in range [0, 65535]'
        );
        return;
      ...
      case 'long':
        ...
        assert_true(
          ...
          'long ' + value + ' not in range [-2147483648, 2147483647]'
        );
        return;
      ...
      case 'unsigned long':
        ...
        assert_true(
          ...
          'unsigned long ' + value + ' not in range [0, 4294967295]'
        );
        return;
  ...
  function IdlInterface(obj, is_callback) {
    ...
    if (this.has_extended_attribute('Unforgeable')) {
      this.members
        .filter(function(m) {
          return (
            !m['static'] && (m.type == 'attribute' || m.type == 'operation')
          );
        })
        .forEach(function(m) {
  ...
  IdlInterface.prototype.is_global = function() //@{
  {
    return this.extAttrs.some(function(attribute) {
      return attribute.name === 'Global' || attribute.name === 'PrimaryGlobal';
    });
  };
  //@}
  ...
  IdlInterface.prototype.test_self = function() //@{
  {
    test(
      function() {
        ...
        assert_false(
          'get' in desc,
          "self's property " + format_value(this.name) + ' has getter'
        );
        assert_false(
          'set' in desc,
          "self's property " + format_value(this.name) + ' has setter'
        );
        assert_true(
          desc.writable,
          "self's property " + format_value(this.name) + ' is not writable'
        );
        assert_false(
          desc.enumerable,
          "self's property " + format_value(this.name) + ' is enumerable'
        );
        assert_true(
          desc.configurable,
          "self's property " + format_value(this.name) + ' is not configurable'
        );
        ...
        if (this.is_callback()) {
          ...
          assert_equals(
            Object.getPrototypeOf(self[this.name]),
            Object.prototype,
        ...
        if (this.base) {
          ...
          //    interface."
          var has_interface_object = !this.array.members[
            this.base
    ...
    if (!this.is_callback()) {
      test(
        function() {
          ...
          assert_true(
            desc.configurable,
            this.name + '.length is not configurable'
          );
    ...
    if (!this.is_callback() || this.has_constants()) {
      test(
        function() {
          ...
          assert_true(
            desc.configurable,
            this.name + '.name is not configurable'
          );
          assert_equals(
            self[this.name].name,
            this.name,
    ...
    test(
      function() {
        ...
        assert_false(
          desc.configurable,
          this.name + '.prototype is configurable'
        );
        ...
        if (this.name === 'Window') {
          ...
          if (this.base) {
            ...
            inherit_interface_has_interface_object = !this.array.members[
              inherit_interface
            ].has_extended_attribute('NoInterfaceObject');
    ...
    test(
      function() {
        ...
        assert_false(
          'get' in desc,
          this.name + '.prototype.constructor has getter'
        );
        assert_false(
          'set' in desc,
          this.name + '.prototype.constructor has setter'
        );
        assert_true(
          desc.writable,
        ...
        assert_equals(
          self[this.name].prototype.constructor,
          self[this.name],
          this.name +
  ...
  {
    var a_test = async_test(
      this.name +
        ' interface: operation ' +
        member.name +
        '(' +
        member.arguments.map(function(m) {
  ...
  ) //@{
  {
    ...
    if (
      !this.has_extended_attribute('NoInterfaceObject') &&
  ...
  ) //@{
  {
    ...
    assert_false(
      'value' in desc,
      'property descriptor has value but is supposed to be accessor'
    );
    assert_false(
      'writable' in desc,
      'property descriptor has "writable" field but is supposed to be accessor'
    );
    assert_true(desc.enumerable, 'property is not enumerable');
    if (member.isUnforgeable) {
      assert_false(
        desc.configurable,
        '[Unforgeable] property must not be configurable'
      );
    } else {
