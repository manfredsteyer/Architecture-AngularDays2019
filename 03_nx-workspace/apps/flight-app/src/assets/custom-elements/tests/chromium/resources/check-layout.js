...
(function() {
  function insertAfter(nodeToAdd, referenceNode) {
    ...
    else referenceNode.parentNode.appendChild(nodeToAdd);
  }
  ...
  function checkExpectedValues(node, failures) {
    ...
    if (expectedWidth) {
      if (
        isNaN(expectedWidth) ||
        Math.abs(node.offsetWidth - expectedWidth) >= 1
      )
        failures.push(
          'Expected ' +
            expectedWidth +
    ...
    if (expectedHeight) {
      if (
        ...
        Math.abs(node.offsetHeight - expectedHeight) >= 1
      )
        failures.push(
          'Expected ' +
            expectedHeight +
    ...
    if (expectedOffset) {
      if (
        isNaN(expectedOffset) ||
        Math.abs(node.offsetLeft - expectedOffset) >= 1
      )
        failures.push(
          'Expected ' +
            expectedOffset +
            ' for offsetLeft, but got ' +
            node.offsetLeft +
            '. '
        );
    }
    ...
    if (expectedOffset) {
      if (
        isNaN(expectedOffset) ||
        Math.abs(node.offsetTop - expectedOffset) >= 1
      )
        failures.push(
          'Expected ' +
            expectedOffset +
            ' for offsetTop, but got ' +
            node.offsetTop +
            '. '
        );
    }

    var expectedOffset = checkAttribute(
      output,
      node,
      'data-positioned-offset-x'
    );
    if (expectedOffset) {
      var actualOffset =
        node.getBoundingClientRect().left -
        positionedAncestor(node).getBoundingClientRect().left;
      if (isNaN(expectedOffset) || Math.abs(actualOffset - expectedOffset) >= 1)
        failures.push(
          'Expected ' +
            expectedOffset +
            ' for getBoundingClientRect().left offset, but got ' +
            actualOffset +
            '. '
        );
    }
    ...
    var expectedOffset = checkAttribute(
      output,
      node,
      'data-positioned-offset-y'
    );
    if (expectedOffset) {
      var actualOffset =
        node.getBoundingClientRect().top -
        positionedAncestor(node).getBoundingClientRect().top;
      if (isNaN(expectedOffset) || Math.abs(actualOffset - expectedOffset) >= 1)
        failures.push(
          'Expected ' +
            expectedOffset +
            ' for getBoundingClientRect().top offset, but got ' +
            actualOffset +
            '. '
        );
    }

    var expectedWidth = checkAttribute(
      output,
      node,
    ...
    if (expectedWidth) {
      if (
        isNaN(expectedWidth) ||
        Math.abs(node.clientWidth - expectedWidth) >= 1
      )
        failures.push(
          'Expected ' +
            expectedWidth +
            ' for clientWidth, but got ' +
            node.clientWidth +
            '. '
        );
    }

    var expectedHeight = checkAttribute(
      output,
      node,
    ...
    if (expectedHeight) {
      if (
        isNaN(expectedHeight) ||
        Math.abs(node.clientHeight - expectedHeight) >= 1
      )
        failures.push(
          'Expected ' +
            expectedHeight +
            ' for clientHeight, but got ' +
            node.clientHeight +
            '. '
        );
    }
    ...
    if (expectedWidth) {
      if (
        isNaN(expectedWidth) ||
        Math.abs(node.scrollWidth - expectedWidth) >= 1
      )
        failures.push(
          'Expected ' +
            expectedWidth +
            ' for scrollWidth, but got ' +
            node.scrollWidth +
            '. '
        );
    }
    ...
    if (expectedHeight) {
      if (
        isNaN(expectedHeight) ||
        Math.abs(node.scrollHeight - expectedHeight) >= 1
      )
        failures.push(
          'Expected ' +
            expectedHeight +
            ' for scrollHeight, but got ' +
            node.scrollHeight +
            '. '
        );
    }
    ...
    if (expectedOffset) {
      ...
      if (isNaN(expectedOffset) || Math.abs(totalLeft - expectedOffset) >= 1)
        failures.push(
          'Expected ' +
            expectedOffset +
            ' for clientLeft+offsetLeft, but got ' +
            totalLeft +
            ', clientLeft: ' +
            node.clientLeft +
            ', offsetLeft: ' +
    ...
    if (expectedOffset) {
      ...
      if (isNaN(expectedOffset) || Math.abs(totalTop - expectedOffset) >= 1)
        failures.push(
          'Expected ' +
            expectedOffset +
            ' for clientTop+offsetTop, but got ' +
            totalTop +
            ', clientTop: ' +
            node.clientTop +
            ', + offsetTop: ' +
    ...
    var expectedPaddingTop = checkAttribute(
      output,
      node,
    ...
    if (expectedPaddingTop) {
      ...
      actualPaddingTop = actualPaddingTop.substring(
        0,
        actualPaddingTop.length - 2
      );
      if (actualPaddingTop != expectedPaddingTop)
        failures.push(
          'Expected ' +
            expectedPaddingTop +
            ' for padding-top, but got ' +
            actualPaddingTop +
            '. '
        );
    }
    ...
    if (expectedPaddingBottom) {
      ...
      // Trim the unit "px" from the output.
      actualPaddingBottom = actualPaddingBottom.substring(
        0,
        actualPaddingBottom.length - 2
      );
      if (actualPaddingBottom != expectedPaddingBottom)
        failures.push(
          'Expected ' +
            expectedPaddingBottom +
            ' for padding-bottom, but got ' +
            actualPaddingBottom +
            '. '
        );
    }

    var expectedPaddingLeft = checkAttribute(
      output,
      node,
    ...
    if (expectedPaddingLeft) {
      ...
      // Trim the unit "px" from the output.
      actualPaddingLeft = actualPaddingLeft.substring(
        0,
        actualPaddingLeft.length - 2
      );
      if (actualPaddingLeft != expectedPaddingLeft)
        failures.push(
          'Expected ' +
            expectedPaddingLeft +
            ' for padding-left, but got ' +
            actualPaddingLeft +
            '. '
        );
    }

    var expectedPaddingRight = checkAttribute(
      output,
      node,
    ...
    if (expectedPaddingRight) {
      ...
      // Trim the unit "px" from the output.
      actualPaddingRight = actualPaddingRight.substring(
        0,
        actualPaddingRight.length - 2
      );
      if (actualPaddingRight != expectedPaddingRight)
        failures.push(
          'Expected ' +
            expectedPaddingRight +
            ' for padding-right, but got ' +
            actualPaddingRight +
            '. '
        );
    }

    var expectedMarginTop = checkAttribute(
      output,
      node,
    ...
    if (expectedMarginTop) {
      ...
      // Trim the unit "px" from the output.
      actualMarginTop = actualMarginTop.substring(
        0,
        actualMarginTop.length - 2
      );
      if (actualMarginTop != expectedMarginTop)
        failures.push(
          'Expected ' +
            expectedMarginTop +
            ' for margin-top, but got ' +
            actualMarginTop +
            '. '
        );
    }

    var expectedMarginBottom = checkAttribute(
      output,
      node,
    ...
    if (expectedMarginBottom) {
      ...
      // Trim the unit "px" from the output.
      actualMarginBottom = actualMarginBottom.substring(
        0,
        actualMarginBottom.length - 2
      );
      if (actualMarginBottom != expectedMarginBottom)
        failures.push(
          'Expected ' +
            expectedMarginBottom +
            ' for margin-bottom, but got ' +
            actualMarginBottom +
            '. '
        );
    }

    var expectedMarginLeft = checkAttribute(
      output,
      node,
    ...
    if (expectedMarginLeft) {
      ...
      // Trim the unit "px" from the output.
      actualMarginLeft = actualMarginLeft.substring(
        0,
        actualMarginLeft.length - 2
      );
      if (actualMarginLeft != expectedMarginLeft)
        failures.push(
          'Expected ' +
            expectedMarginLeft +
            ' for margin-left, but got ' +
            actualMarginLeft +
            '. '
        );
    }

    var expectedMarginRight = checkAttribute(
      output,
      node,
    ...
    if (expectedMarginRight) {
      ...
      // Trim the unit "px" from the output.
      actualMarginRight = actualMarginRight.substring(
        0,
        actualMarginRight.length - 2
      );
      if (actualMarginRight != expectedMarginRight)
        failures.push(
          'Expected ' +
            expectedMarginRight +
            ' for margin-right, but got ' +
            actualMarginRight +
            '. '
        );
    }
  ...
  window.checkLayout = function(selectorList, outputContainer) {
    ...
    Array.prototype.forEach.call(nodes, function(node) {
      ...
      var container =
        node.parentNode.className == 'container' ? node.parentNode : node;
    ...
    if (!checkedLayout) {
      document.body.innerHTML =
        'FAIL: No valid data-* attributes found in selector list : ' +
        selectorList;
      return false;
    }
    ...
    if (pre)
      setTimeout(function() {
        pre.previousSibling.scrollIntoView();
      }, 0);
    return result;
  };
})();
