...
if (self.testRunner) {
  if (self.enablePixelTesting) testRunner.dumpAsTextWithPixelResults();
  else testRunner.dumpAsText();
}
...
(function() {
  getOrCreateTestElement = function(id, tagName) {
    ...
    if (id == 'description') refNode = getOrCreateTestElement('console', 'div');
    else refNode = parent.firstChild;
  ...
  description = function description(msg, quiet) {
    ...
    if (quiet)
      span.innerHTML =
        '<p>' +
        msg +
        '</p><p>On success, you will see no "<span class="fail">FAIL</span>" messages, followed by "<span class="pass">TEST COMPLETE</span>".</p>';
    else
      span.innerHTML =
        '<p>' +
        msg +
    ...
    else description.appendChild(span);
  };
  ...
  function handleTestFinished() {
    ...
    if (window.jsTestIsAsync) {
      ...
      if (window.wasFinishJSTestCalled) finishJSTest();
    } else finishJSTest();
  }
...
function descriptionQuiet(msg) {
  description(msg, true);
}
...
function isResultCorrect(actual, expected) {
  if (expected === 0) return actual === expected && 1 / actual === 1 / expected;
...
function stringify(v) {
  if (isNewSVGTearOffType(v)) return v.valueAsString;
...
function shouldBe(_a, _b, quiet, opt_tolerance) {
  ...
  if (_exception)
    ...
    isResultCorrect(_av, _bv) ||
    (typeof opt_tolerance == 'number' &&
      typeof _av == 'number' &&
...
function shouldBecomeEqual(_a, _b, _completionHandler, _timeout) {
  ...
  if (_timeout === undefined) _timeout = 500;
  ...
  var _condition = function() {
    ...
    if (_exception)
      testFailed(
        _a + ' should become ' + _bv + '. Threw exception ' + _exception
      );
...
function shouldBecomeEqualToString(
  value,
  reference,
  completionHandler,
  timeout
...
function shouldBeCloseTo(_a_raw, _b_raw, _tolerance, _quiet) {
  if (typeof _a_raw != 'string') {
    testFailed(
      'shouldBeCloseTo() requires string argument _a_raw. was type ' +
        typeof _a_raw
    );
    return;
  }
  if (typeof _b_raw != 'number' && typeof _b_raw != 'string') {
    testFailed(
      'shouldBeCloseTo() requires numeric or string argument _b_raw. was type ' +
        typeof _b_raw
    );
    return;
  }
  if (typeof _tolerance != 'number') {
    testFailed(
      'shouldBeCloseTo() requires numeric argument _tolerance. was type ' +
        typeof _tolerance
    );
    return;
  }
...
function shouldNotBe(_a, _b, _quiet) {
  ...
  if (_exception)
    testFailed(
      _a + ' should not be ' + _bv + '. Threw exception ' + _exception
    );
  else if (!isResultCorrect(_av, _bv)) {
    if (!_quiet) {
      testPassed(_a + ' is not ' + _b);
    }
  } else testFailed(_a + ' should not be ' + _bv + '.');
}
...
function shouldBeTrue(a, quiet) {
  shouldBe(a, 'true', quiet);
}
function shouldBeTrueQuiet(a) {
  shouldBe(a, 'true', true);
}
function shouldBeFalse(a, quiet) {
  shouldBe(a, 'false', quiet);
}
function shouldBeNaN(a, quiet) {
  shouldBe(a, 'NaN', quiet);
}
function shouldBeNull(a, quiet) {
  shouldBe(a, 'null', quiet);
}
function shouldBeZero(a, quiet) {
  shouldBe(a, '0', quiet);
}
...
function shouldBeEmptyString(a) {
  shouldBeEqualToString(a, '');
}
...
function shouldEvaluateTo(actual, expected, opt_tolerance) {
  ...
  if (expected == null) {
    ...
    if (expected) shouldBeTrue(actual);
    else shouldBeFalse(actual);
  } else if (typeof expected == 'number') {
    if (opt_tolerance) shouldBeCloseTo(actual, expected, opt_tolerance);
    else shouldBe(actual, stringify(expected));
  } else {
...
function shouldEvaluateToSameObject(actual, expected, quiet) {
  if (typeof actual != 'string')
    debug(
      'WARN: shouldEvaluateToSameObject() expects the first argument (actual) to be a string.'
    );
  try {
  ...
  if (isResultCorrect(actualEvaled, expected)) {
    if (!quiet) testPassed(actual + ' is ' + stringify(expected));
  } else {
    testFailed(
      actual +
        ' should be ' +
...
function shouldBeNonZero(_a) {
  ...
  else if (_av != 0) testPassed(_a + ' is non-zero.');
  else testFailed(_a + ' should be non-zero. Was ' + _av);
}
...
function shouldBeNonNull(_a) {
  ...
  else if (_av != null) testPassed(_a + ' is non-null.');
  else testFailed(_a + ' should be non-null. Was ' + _av);
}
...
function shouldBeUndefined(_a) {
  ...
  else if (typeof _av == 'undefined') testPassed(_a + ' is undefined.');
  else testFailed(_a + ' should be undefined. Was ' + _av);
}
...
function shouldBeDefined(_a) {
  ...
  else if (_av !== undefined) testPassed(_a + ' is defined.');
  else testFailed(_a + ' should be defined. Was ' + _av);
}
  ...
  if (_exception) {
    if (typeof _e == 'undefined' || _exception == _ev)
      ...
      testFailed(
        _a +
          ' should throw ' +
          (typeof _e == 'undefined' ? 'an exception' : _ev) +
          '. Threw exception ' +
          _exception +
          '.'
      );
...
function shouldBeNow(a, delta) {
  ...
  testFailed(
    a +
      ' cannot be tested against the current time. The clock is going backwards too often.'
  );
}
...
function shouldHaveHadError(message) {
  if (expectingError) {
    testFailed(
      'No error thrown between expectError() and shouldHaveHadError()'
    );
    return;
  }

  if (expectedErrorMessage) {
    if (!message) testPassed('Got expected error');
    else if (expectedErrorMessage.indexOf(message) !== -1)
      testPassed("Got expected error: '" + message + "'");
    else testFailed("Unexpected error '" + message + "'");
...
function asyncGC(callback) {
  ...
  setTimeout(function() {
    ...
    if (documentsAfter < documentsBefore) asyncGC(callback);
    else callback();
  }, 0);
}

function gc() {
  if (typeof GCController !== 'undefined') GCController.collectAll();
  else {
...
function asyncMinorGC(callback) {
  if (typeof GCController !== 'undefined') GCController.minorCollect();
  else
    testFailed(
      'Minor GC is available only when you enable the --expose-gc option in V8.'
    );
  // FIXME: we need a better way of waiting for chromium events to happen
...
function finishJSTest() {
  ...
  if (!self.wasPostTestScriptParsed) return;
  isSuccessfullyParsed();

  if (self._lazyDescription)
    getOrCreateTestElement('description', 'p').appendChild(
      self._lazyDescription
    );
  ...
  if (self.jsTestIsAsync && self.testRunner) testRunner.notifyDone();
}
...
function startWorker(testScriptURL, workerType) {
  ...
  else worker = new Worker(testScriptURL);
  worker.onmessage = function(event) {
    ...
    if (code == 'PASS') testPassed(payload);
    else if (code == 'FAIL') testFailed(payload);
    else if (code == 'DESC') description(payload);
    else if (code == 'DONE') finishJSTest();
    else debug(workerPrefix + event.data);
  };
  ...
  if (self.name == 'Shared Worker') {
    self.onconnect = function(e) {
      ...
      workerPort.onmessage = function(event) {
        ...
        try {
          if (code == 'IMPORT') importScripts(payload);
          else
            testFailed('Unrecognized message to shared worker: ' + event.data);
        } catch (ex) {
