...
Markup.dump = function(opt_node, opt_description) {
  if (typeof opt_node == 'string') opt_node = document.getElementById(opt_node);
  ...
  if (Markup._dumpCalls == 2 && Markup._firstCallDidNotHaveDescription) {
    var wrapper = Markup._container.getElementsByClassName(
      'dump-as-markup-span'
    )[0];
  ...
  if (Markup._test_description && Markup._dumpCalls == 1)
    Markup._container.appendChild(
      document.createTextNode(Markup._test_description + '\n')
    );
...
Markup.notifyDone = function() {
  ...
  if (!Markup._dumpCalls) Markup.dump();
...
Markup.get = function(node) {
  ...
  if (markup) return markup.substring(1);
  ...
  switch (node.nodeType) {
    ...
    case Node.ELEMENT_NODE:
      ...
      if (node.localName && node.namespaceURI && node.namespaceURI != null)
        str += node.localName;
      else str += Markup._toAsciiLowerCase(node.nodeName);
      ...
      if (!Markup._useHTML5libOutputFormat && window.internals) {
        ...
        if (pseudoId)
          str +=
            Markup._indent(depth + 1) + 'shadow:pseudoId="' + pseudoId + '"';
      }
...
Markup._getShadowHostIfPossible = function(node, depth, shadowRootList) {
  if (
    !Markup._useHTML5libOutputFormat &&
...
Markup._getMarkupForTextNode = function(node) {
  ...
  if (startText && endText)
    innerMarkup =
      innerMarkup.substring(0, startOffset) +
      startText +
      innerMarkup.substring(startOffset, endOffset) +
      endText +
      innerMarkup.substring(endOffset);
  else if (startText)
    innerMarkup =
      innerMarkup.substring(0, startOffset) +
      startText +
      innerMarkup.substring(startOffset);
...
Markup._getSelectionMarker = function(node, index) {
  ...
  if (index == sel.anchorOffset && node == sel.anchorNode) {
    if (sel.isCollapsed) return Markup._SELECTION_CARET;
    else return Markup._SELECTION_ANCHOR;
