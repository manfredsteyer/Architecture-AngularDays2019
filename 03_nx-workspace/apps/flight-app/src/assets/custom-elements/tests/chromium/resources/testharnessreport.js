...
(function() {
  ...
  function injectSyntheticInput() {
    ...
    if (path.match(/imported\/wpt\/.*\.html$/)) {
      ...
      var automated_input_scripts_folder = path.replace(
        /imported\/wpt\/(.*)\.html$/,
      ...
      importAutomationScript = function(relativePath) {
        ...
        common_script.setAttribute(
          'src',
          automated_input_scripts_folder + relativePath
        );
      ...
      path = path.replace(
        /imported\/wpt\/(.*)\.html$/,
        'imported/wpt_automation/$1-automation.js'
      );
  ...
  add_completion_callback(function(tests, harness_status) {
    ...
    if (harness_status.status != 0) {
      resultStr +=
        'Harness Error. harness_status.status = ' +
    ...
    function done() {
      ...
      if (!document.body || document.body.tagName != 'BODY') {
        if (!document.documentElement)
          ...
          // document.body is <frameset>.
          document.body.remove();
        document.documentElement.appendChild(document.createElement('body'));
      }
      document.body.appendChild(results);

      if (self.testRunner) testRunner.notifyDone();
    }
