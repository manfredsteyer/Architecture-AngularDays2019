...
Magnitude.successThreshold = 0.5;
...
Magnitude._sortTrimArray = function(array, trim) {
  ...
  var sorted_array = array.map(Math.abs).sort(function(a, b) {
    return a - b;
  });
  if (!trim) return sorted_array;
  else return sorted_array.slice(trim, -trim);
};
...
Magnitude._for = function(n, body, callback) {
  ...
  function iteration(result) {
    results.push(result);
    if (++i === n) callback(results);
    else body(i, iteration);
  }
...
Magnitude._run = function(setup, test, expected, async) {
  ...
  function runTrial(trialNumber, nextTrial) {
    ...
    Magnitude._for(
      Magnitude.numPoints,
      Magnitude._runTime.bind(null, setup, test),
      completeTrial.bind(null, nextTrial)
    );
  }
  function completeTrial(nextTrial, times) {
    ...
    Magnitude._debug(
      'Trial #' +
        Magnitude._trialNumber +
        ': ' +
        (passed ? 'SUCCESS' : 'FAILURE')
    );
    if (passed) Magnitude._numSuccesses++;
    nextTrial();
  }
...
Magnitude._finish = function() {
  ...
  Magnitude._debug(
    'Successes: ' +
      ...
      neededToPass +
      ' (' +
      100 * Magnitude.successThreshold +
      '%) ' +
      'to pass'
  );
  ...
  if (!window.testRunner || !passedOverall) Magnitude._log(Magnitude._debugLog);

  if (Magnitude._async && window.testRunner) testRunner.notifyDone();
};
...
Magnitude._runTime = function(setup, test, pointIndex, callback) {
  ...
  function iteration(nextIteration) {
    ...
    Magnitude._while(
      function() {
        return totalTimeMilliseconds < Magnitude.millisecondsPerRun;
      },
      function(nextRun) {
        test(magnitude, completeRun.bind(null, nextRun));
      },
      completeIteration.bind(null, nextIteration)
    );
  }
  ...
  function completeIteration(nextIteration) {
    ...
    var overTimeTolerance =
      Magnitude.millisecondsPerRun * relativeOverTimeTolerance;
    ...
    if (overTime < millisecondsPerIteration + overTimeTolerance) runOk = true;
    else {
      Magnitude._debug(
        'over-time: ' +
          overTime +
          ' exceeds average run-time ' +
          millisecondsPerIteration +
          ' by more than tolerance of ' +
          overTimeTolerance +
          ' assuming delay'
      );
      attempt++;
      if (attempt < maxAttempts)
        ...
        Magnitude._debug(
          'Failed to reduce delay noise after ' +
            maxAttempts +
            ' attempts, proceeding with noisy data'
        );
        runOk = true;
      }
    }
  ...
  Magnitude._while(
    ...
    iteration,
    function() {
      callback(millisecondsPerIteration);
    }
  );
};
...
Magnitude._isConstant = function() {
  ...
  var trimmedAbsRelativeDeviations = Magnitude._absSortDownTrimTop(
    relativeDeviations,
    Magnitude.trim
  );
  Magnitude._debug(
    'Trimmed sorted absolute relative deviations: ' +
...
Magnitude._isLinear = function() {
  ...
  var maxAbsDeviation = Math.max(
    Math.abs(1 - minLogRatio),
...
window.addEventListener(
  ...
  function() {
    ...
    if (window.testRunner) document.body.innerHTML = '';
